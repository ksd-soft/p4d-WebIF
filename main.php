...
include("header.php");
if (checkMobile() == 1) { header('Location: ' . $_SESSION['smartHome']); }   
...
printHeader(60);
  ...
  $p4dSince = preg_replace("/(Stunden)/i","Std",$p4dSince);
  $p4dSince = preg_replace("/(Minuten)/i","min",$p4dSince);
  ...
  if ($p4dstate == 0)
  {
    ...
    echo  "          <table>\n";
    echo  "            <tr><td width=58%>Läuft seit:</td><td>$p4dSince</td></tr>\n";
    ...
    echo  "            <tr><td>Version:</td><td><font style=font-size:0.8em;>$p4dVersion</font></td></tr>\n";
